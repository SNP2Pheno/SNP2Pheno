...
rs_ids = {"rs12913832" : "A", "rs12896399" : "G", "rs1408799" : "C"}    # currently used ones for clustering => see dendrogram
...
l = []
# map eye color to each user => look for userID and get the eye color
for u in df2.index:
    if u == "jve":
        l.append("Blue")
        continue
...
# select only specific eye colors for better visualization
...
# seaborn clustermap
lut = dict(zip([str(x) for x in np.unique(X["EyeColor"])], ["tab:blue", "brown", "tab:green"])) #  ["tab:blue", "brown"])) # ["tab:blue", "grey", "brown", "tab:green"])) # ["tab:blue", "brown"])) # ["tab:blue", "grey", "brown", "tab:green"]))
...
# Plot t-SNE scatter with points colored by eye color => better separation than PCA
plt.figure(figsize=(10, 7), dpi=400)
